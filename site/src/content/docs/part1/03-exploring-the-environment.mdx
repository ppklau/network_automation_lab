...
<Mermaid code={`
flowchart TD
    A["SoT edit\\nleaf-lon-01.yml"] --> B["git commit\\nGitLab CI triggered"]

    B --> C["Stage 1: validate\\nyamllint · validate_sot.py\\nschemas + cross-references"]
    C -->|pass| D["Stage 2: intent-check\\nBatfish pytest assertions\\nzone isolation · routing policy · BGP standards"]
    C -->|fail| STOP1["Pipeline halted\\nno change deployed"]

    D -->|pass| E["Stage 3: render\\nJinja2 + SoT\\n→ configs/leaf-lon-01/running.conf"]
    D -->|fail| STOP2["Pipeline halted\\nno change deployed"]

    E --> F["Stage 4: diff\\nrunning.conf vs last deployed\\nshows exactly what will change"]

    F --> G["Stage 5: approve\\nEngineer reviews diff\\nclicks Approve in GitLab"]
    G -->|approved| H["Stage 6: push\\nAnsible pushes config atomically\\ncaptures before/after state"]
    G -->|rejected| STOP3["Pipeline halted\\nno change deployed"]

    H --> I["Stage 7: verify\\nBGP neighbors up · interfaces up\\nmanagement reachable\\nartefact stored in GitLab"]
    I -->|fail| J["Rollback triggered\\nprevious config restored"]
    I -->|pass| K["Change complete\\naudit artefact recorded"]
`} />
...
## Exercise 3.2 — Trace a change through the pipeline
