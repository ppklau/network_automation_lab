...
<Mermaid code={`
flowchart TD
    REQ["Layer 1 — Business Requirements\\nrequirements/business_requirements.yml\\n\\n'Trading systems must be isolated\\nfrom corporate users'\\nMiFID II Art. 48 · FCA SYSC 8.1"]
    INTENT["Layer 2 — Design Intents\\ndesign_intents/zone_isolation.yml\\n\\n'No reachability between\\nTRADING_VRF and CORPORATE_VRF'\\n'fw-lon-01 must be on path\\nfor all inter-zone traffic'"]
    SOT["Layer 3 — Operational Values (SoT)\\nsot/sites/fra-dc1.yml — vlans_prohibited: [100]\\nsot/devices/lon-dc1/leaf-lon-01.yml — vrf: TRADING_VRF\\nsot/compliance/zone_policies.yml — ZONE-001"]
    CONFIG["Rendered Configuration\\nconfigs/leaf-lon-01/running.conf\\nvrf TRADING_VRF · no cross-VRF routes"]
    TEST["Batfish Assertions\\nbatfish/tests/test_zone_isolation.py\\nbatfish/tests/test_frankfurt_isolation.py"]

    REQ -->|"translates to"| INTENT
    INTENT -->|"implemented in"| SOT
    SOT -->|"rendered as"| CONFIG
    CONFIG -->|"verified by"| TEST
    TEST -.->|"evidence traces back to"| REQ
`} />
...
## Exercise 6.1 — Write a new design intent
...
Management plane isolation is one of the most commonly violated security intents in financial services networks — not through malice, but because OOB management is treated as an afterthought. Codifying it as an intent (and testing it in Batfish) means a future config push that accidentally exposes SSH to CORPORATE zone fails before it reaches a device.
